...
export default function EmployeesView({
  ...
  // Newly-created employee password reveal
  const [newlyCreated, setNewlyCreated] = useState<{ name: string; id: string; password: string } | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      // password will be auto-generated by App.tsx handler
      password: '',
    };
    ...
    if (isEditing && selectedEmp) {
      ...
      // onAddEmployee returns the created employee with the generated password
      const created = onAddEmployee(payload);
      setNewlyCreated({ name: created.name, id: created.id, password: created.password });
      setCopied(false);
      // Don't close — show the password reveal banner instead
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] transition-colors">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
              ...
              {/* Password reveal banner — shown after new employee is created */}
              {newlyCreated && !isEditing && (
                <div className="rounded-xl border border-emerald-200 dark:border-emerald-800/60 bg-emerald-50 dark:bg-emerald-950/40 p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg bg-emerald-500/15 flex items-center justify-center">
                      <KeyRound className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest">Employee Created!</p>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{newlyCreated.name} · {newlyCreated.id}</p>
                    </div>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Auto-generated login password — share securely with the employee:</p>
                    <div className="flex items-center gap-2">
                      <code className="flex-1 bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800/50 rounded-lg px-3 py-2 text-sm font-mono font-bold text-emerald-700 dark:text-emerald-300 tracking-widest select-all">
                        {newlyCreated.password}
                      </code>
                      <button
                        type="button"
                        onClick={() => handleCopyPassword(newlyCreated.password)}
                        className="p-2 rounded-lg border border-emerald-200 dark:border-emerald-800/50 bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-950/60 transition-all"
                        title="Copy password"
                      >
              ...
              <div className="border-t border-slate-200 dark:border-slate-800 pt-4 flex gap-3">
                <button
                  type="button"
                  onClick={() => { setShowAddForm(false); setNewlyCreated(null); }}
