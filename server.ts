  ...
  if (campaignType === "Birthday") {
    ...
    accentBlock = `
      <div style="margin-top: 20px; padding: 15px; background-color: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 12px; font-family: monospace; font-size: 12px;">
        ...
        <p style="margin: 8px 0 0 0; font-size: 11px; color: #065f46; font-style: italic;">Detailed payslip calculations have been successfully credited. Check your HRMS-CE Portal to view/download PDF.</p>
      </div>
    `;
  } else {
...
// Endpoint: Forgot Password — notifies HR admin via email
...
app.post("/api/email/send", async (req, res) => {
  ...
  if (transporter) {
    ...
    try {
      const info = await transporter.sendMail({
        from: `"${fromName}" <${fromEmail}>`,
        to: recipientEmail,
        subject: subject || `HRMS-CE Campaign: ${campaignType}`,
  ...
  try {
    ...
    if (freeTransporter) {
      const info = await freeTransporter.sendMail({
        from: '"HRMS-CE Auto-SMTP" <no-reply@hrms-ce.local>',
        to: recipientEmail,
        subject: subject || `HRMS-CE Campaign: ${campaignType}`,
        html: generateEmailHtml(campaignType, employeeName || "Employee", subject, body || ""),
      });
...
app.post("/api/ai/draft", async (req, res) => {
  ...
  if (!client) {
    ...
    const simulatedSubject = `HRMS-CE Automatic Draft: ${type} for ${employeeName || "Team Member"}`;
    let simulatedBody = "";
    if (type === "Offer Letter") {
      simulatedBody = `Dear ${employeeName || "[Candidate Name]"},\n\nWe are absolutely delighted to offer you the position of ${role || "[Role]"} in our ${department || "[Department]"} department at HRMS-CE. We were incredibly impressed by your interviews, skills, and values.\n\nKey Terms of the Offer:\n- Position: ${role || "[Role]"}\n- Department: ${department || "[Department]"}\n- Annual Salary: ${extraDetails || "$95,000"}\n- Start Date: Immediately\n\n(Simulated draft. To enable real-time Gemini generation, please configure your GEMINI_API_KEY in the Secrets panel)`;
    } else if (type === "LOR") {
...
async function start() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`HRMS-CE Server listening on http://0.0.0.0:${PORT}`);
  });
}
